    ...
    for iter in range(model_para['iterations']):
        ...
        while (batch_no + 1) * batch_size < train_set.shape[0]:
            ...
            if numIters % args.eval_iter == 0:
                # print("-------------------------------------------------------train1")
                print("train LOSS: %.4f\tITER: %d\tBATCH_NO: %d\tSTEP: %d\t total_batches: %d" %
            ...
            if numIters % args.eval_iter == 0:
                ...
                print("test LOSS: %.4f\tITER: %d\tBATCH_NO: %d\tSTEP: %d\t total_batches: %d" %
                      (np.mean(loss), iter, batch_no, numIters, valid_set.shape[0] / batch_size))
            ...
            if numIters % args.eval_iter == 0:
                ...
                while (batch_no_test + 1) * batch_size_test < valid_set.shape[0]:
                    ...
                    thismrr5 = sum(curr_preds_5) / float(len(curr_preds_5))
                    thismrr20 = sum(curr_preds_20) / float(len(curr_preds_20))
                    thishit5 = sum(rec_preds_5) / float(len(rec_preds_5))
                    thishit20 = sum(rec_preds_20) / float(len(rec_preds_20))
                    thisndcg5 = sum(ndcg_preds_5) / float(len(ndcg_preds_5))
                    thisndcg20 = sum(ndcg_preds_20) / float(len(ndcg_preds_20))
                ...
                print("\t\tmax_mrr_20=%.4f  max_hit_20=%.4f  max_ndcg_20=%.4f\n" %
                    ...
                    # print("mrr_5:", sum(curr_preds_5) / float(len(curr_preds_5)))  # 5
                    # print("mrr_20:", sum(curr_preds_20) / float(len(curr_preds_20)))  # 5
                    # print("hit_5:", sum(rec_preds_5) / float(len(rec_preds_5)))  # 5
                    # print("hit_20:", sum(rec_preds_20) / float(len(rec_preds_20)))  # 5
                    # print("ndcg_5:", sum(ndcg_preds_5) / float(len(ndcg_preds_5)))  # 5
                    # print("ndcg_20:", sum(ndcg_preds_20) / float(len(ndcg_preds_20)))  #
