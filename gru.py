...
def main(datapath=None):
    ...
    parser.add_argument('--text_dir', type=str,
                        default='Data/Session/short.csv',
    ...
    print("embedding_size:", args.embedding_dim)
    ...
    for epoch in range(args.max_epochs):
        ...
        while (batch_no_test + 1) * batch_size < len(x_dev):
            ...
            test_batch = x_dev[batch_no_test * batch_size: (batch_no_test + 1) * batch_size, :]
            # [probs], loss = sess.run(
            probs, loss = sess.run(
                [rnn.probs_flat, rnn.loss],
                feed_dict={
                    rnn.wholesession: test_batch,
...
if __name__ == '__main__':
    ...
    t2 = time.time()
    print("grurec %s time = %.2f mins" % (data, (t2 - t1) / 60))
